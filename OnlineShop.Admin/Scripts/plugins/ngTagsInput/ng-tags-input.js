...
tagsInput.directive('autoComplete', ["$document", "$timeout", "$sce", "$q", "tagsInputConfig", "tiUtil", function($document, $timeout, $sce, $q, tagsInputConfig, tiUtil) {
    ...
    return {
        ...
        controller: ["$scope", "$element", "$attrs", function ($scope, $element, $attrs) {
            $scope.events = tiUtil.simplePubSub();
            var loadOdEmpty = false;

            if ($attrs.loadOnEmpty && ($attrs.loadOnEmpty === 'true' || $attrs.loadOnEmpty === 'True')) {
                loadOdEmpty = true;
            }

            var loadOnFocus = false;

            if ($attrs.loadOnFocus && ($attrs.loadOnFocus === 'true' || $attrs.loadOnFocus === 'True')) {
                loadOnFocus = true;
            }
            ...
            tagsInputConfig.load('autoComplete', $scope, $attrs, {
                ...
                loadOnEmpty: [Boolean, loadOdEmpty],
                loadOnFocus: [Boolean, loadOnFocus],
        ...
        link: function(scope, element, attrs, tagsInputCtrl) {
            ...
            scope.addSuggestion = function() {
                ...
                if (suggestionList.selected) {
                    ...
                    added = true;
                    var parent = $(element[0]).parents('input');
                }
                return added;
            };
...
tagsInput.run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put('ngTagsInput/tag-item.html',
    "<span ng-bind-html=\"$getDisplayText()\"></span> <a class=\"remove-button\" ng-click=\"$removeTag()\" ng-bind=\"::$$removeTagSymbol\"></a>"
  );
